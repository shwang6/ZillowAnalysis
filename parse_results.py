...
"""
file = open("search-results-sample.txt", "r") 
response = file.read()
file.close()
...
print("There are", nListings[0].text, nListings[0].next_sibling, "and", nListings[1].text, nListings[1].next_sibling)

#Get data from all listings
all_listings = soup.find('script', attrs={"data-zrr-shared-data-key": "mobileSearchPageStore"}).string
...
pprint.pprint(all_listings['cat1']['searchResults']['listResults']) #This is the file 'search-results-sample-cleaned.txt'
...
"""
file = open('individual-listing-sample.txt', 'r')
response = file.read()
file.close()
soup = BeautifulSoup(response, "html.parser")

#Get data from listing
data = json.loads(soup.find("script", type="application/json", id="hdpApolloPreloadedData").text)
